...
function get_libuv () {
    ...
    if [ -f "$LIBUV_ARCH_PATH" ]; then
        mkdir -p $ARCHIVE_PATH
        mk_softlink_to_dest $LIBUV_ARCH_PATH $ARCHIVE_PATH/$LIBUV_FILE_NAME
        return
    else
        tget_and_rename https://github.com/libuv/libuv/archive/refs/tags/v${CONFIG_LIBUV_VERSION}.tar.gz $LIBUV_FILE_NAME
    fi
}
